from utils.common import np, copy, Tuple
...
class Uniform(Initializer):
    ...
    def __call__(self, shape, **kwargs):
        return Tensor(np.random.uniform(-self.scale, self.scale, size=shape), **kwargs)
...
class Normal(Initializer):
    ...
    def __call__(self, shape, **kwargs):
        return Tensor(np.random.normal(loc=self.mean, scale=self.std, size=shape), **kwargs)
...
class RandInt(Initializer):
    def __call__(self, low, high=None, shape=None, **kwargs):
        return Tensor(np.random.randint(low=low, high=high, size=shape), **kwargs)
...
def ones_like(a, **kwargs):
    ...
    return Tensor(np.ones_like(a.data), requires_grad=requires_grad, **kwargs)
...
def zeros_like(a, **kwargs):
    requires_grad = kwargs.pop('requires_grad', a.requires_grad)
    return Tensor(np.zeros_like(a.data), requires_grad=requires_grad, **kwargs)

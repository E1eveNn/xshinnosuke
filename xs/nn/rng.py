...
class Uniform(Initializer):
    ...
    def __call__(self, shape, **kwargs):
        return core.base.Tensor(np.random.uniform(-self.scale, self.scale, size=shape), **kwargs)
...
class Normal(Initializer):
    ...
    def __call__(self, shape, **kwargs):
        return core.base.Tensor(np.random.normal(loc=self.mean, scale=self.std, size=shape), **kwargs)
...
class Zeros(Initializer):
    def __call__(self, shape, **kwargs):
        return core.base.Tensor(np.zeros(shape), **kwargs)


class Ones(Initializer):
    def __call__(self, shape, **kwargs):
        return core.base.Tensor(np.ones(shape), **kwargs)
...
class Matrix(Initializer):
    ...
    def __call__(self, shape: Tuple, **kwargs):
        return core.base.Tensor(np.ones(shape) * self.value, **kwargs)


class RandN(Initializer):
    def __call__(self, shape: Tuple, **kwargs):
        return core.base.Tensor(np.random.randn(*shape), **kwargs)


class Rand(Initializer):
    def __call__(self, shape: Tuple, **kwargs):
        return core.base.Tensor(np.random.rand(*shape), **kwargs)


class RandInt(Initializer):
    def __call__(self, low, high=None, shape=None, **kwargs):
        return core.base.Tensor(np.random.randint(low=low, high=high, size=shape), **kwargs)
...
def ones_like(a, **kwargs):
    ...
    return core.base.Tensor(np.ones_like(a.data), requires_grad=requires_grad, **kwargs)
...
def zeros_like(a, **kwargs):
    requires_grad = kwargs.pop('requires_grad', a.requires_grad)
    return core.base.Tensor(np.zeros_like(a.data), requires_grad=requires_grad, **kwargs)
